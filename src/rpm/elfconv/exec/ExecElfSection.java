		...
		for (ELFSymbolSection.ELFSymbol s : symbTable.symbols) {
			if (s.sectionIndex == id) {
				if (s.getSymType() == ELFSymbolSection.ELFSymbolType.FUNC) {
					...
					f.absoluteAddress = MathEx.padIntegerDownPow2(f.absoluteAddress, 1); //hword align
					functions.add(f);
				}
			}
		}
						...
						while (buf.getPosition() < (s.endOffset - 2)) {
							...
							if ((instruction & ThumbAssembler.BL_MASK) == ThumbAssembler.BL_HIGH_IDENT) {
								...
								if (externalOffset != 0) {
									...
									r.target = new RPMRelocationTarget(pos, RPMRelTargetType.THUMB_BRANCH_LINK);
									r.source = new RPMRelocationSource(rpm, findRPMFuncByAddr(externalOffset));
									relocs.add(r);
								}
								else {
									RPMRelocation r = new RPMRelocation();
									r.target = new RPMRelocationTarget(pos, RPMRelTargetType.THUMB_BRANCH_LINK);
									r.source = new RPMRelocationSource(rpm, findRPMFuncByAddr(brnchAddr));
									relocs.add(r);
								}
							}
						}
